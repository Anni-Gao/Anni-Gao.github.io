...
(function () {
  ...
  function loadSearchData() {
    ...
    fetch(searchUrl)
      ...
      .then(function (data) {
        ...
        isLoading = false;
        searchResults.innerHTML =
          '<div class="search-tip">Enter the keywords to search</div>';
      })
      .catch(function (error) {
        console.error("Failed to load search data:", error);
        isLoading = false;
        searchResults.innerHTML =
          '<div class="search-error">Please refresh and try again.</div>';
      });
  }
  ...
  function renderResults(results, keywords) {
    if (results.length === 0) {
      searchResults.innerHTML =
        '<div class="search-empty">No relevant results</div>';
      return;
    }
